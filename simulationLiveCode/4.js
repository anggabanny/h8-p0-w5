...
function train(pokemon, candy) {
  var temp = (pokemon.level += candy)
  
  if (pokemon.evolutionLine == undefined) {
    return 'Congratulations, your Charmander grew to LV. ' + temp
  }

  var temp1 = ''
  for(var i = 0 ; i < pokemon.evolutionLine.length; i++){
    if (i == pokemon['evolutionLine'].length-1){
      if (pokemon.evolutionLine[i].level <= temp) {
        temp1+=  ' and ' + pokemon.evolutionLine[i].name
      }
    } else {
      if (pokemon.evolutionLine[i].level <= temp) {
        temp1+=pokemon.evolutionLine[i].name
      }
    }
    
  }
  
  return 'Congratulations, your Charmander grew to LV. ' + temp + '! And it evolved into ' + temp1
}
