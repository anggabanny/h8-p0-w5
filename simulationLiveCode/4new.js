...
function train(pokemon, candy) {
    ...
    var evolution = pokemon.evolutionLine
    console.log(yourLevel);
    var evolved = ''
    if (!evolution) {
        return 'Congratulations, your '+yourPokemon+' grew to LV. '+yourLevel+'!'
    } else {
        for(var i = 0 ; i < evolution.length; i++){
            console.log(evolution[i].level);
            if (yourLevel >= evolution[i].level && evolved.length !==0){
                evolved += ' and ' + evolution[i].name
            } else if (yourLevel >= evolution[i].level) {
                evolved += evolution[i].name
            }
        }
        if (!evolved) {
            return 'Congratulations, your '+yourPokemon +' grew to LV. '+ yourLevel+'!'
        }
        return 'Congratulations, your '+yourPokemon +' grew to LV. '+ yourLevel+'! And it evolved into ' + evolved + '!'
    }
}
var charmander = {
    name: 'Charmander',
...
console.log(train(charmander, 1))
